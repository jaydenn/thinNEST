...
//
//////////////////////////////  IMPORTANT !! //////////////////////////////////////
//  To match our calibration data, we needed to reduce the ER and NR band widths.
//  To make these changes in your copy of NEST, make sure NEST.cpp is calling an 
//  edited NRERWidthsParams vector when using the GetQuanta(...) function:
//  {0.4.,0.4.,0.04,0.5,0.19,2.25, 0.0015, 0.046452, 0.205, 0.45, -0.2};
//
//  If using execNEST to simulate results, these changes can be made manually 
//  to execNEST.cpp at lines 183-194 in NESTv2.3.7
//
//  Additonally, we disabled skewed-recombination to better match our tritium data
//  To make this change in v2.3.7 or earlier, replace lines 353-358 of NEST.cpp 
//  with "skewness = 0.0;"
//
//  as of NESTv2.3.8, the GetQuanta(...) function takes skewness to be a function input.
//  If using this version of NEST, simply make sure GetQuanta uses "skewnessER = 0.0"
//  for ER simulations (this variable is the last argument to GetQuanta).  
///////////////////////////////////////////////////////////////////////////////////
//
//  To add this file into execNEST, make sure it is copied into the directory 
//                        nest/include/Detectors/
//  and add the line ' #include "LZ_SR1.hh" ' , and make sure the "auto detector" 
//  variable is the "LZ_Detector()" object, as opposed to the LUX default. 
//  You may also want to remove the few lines of LUX-detector comments between 
//  lines 42-45 of execNEST.cpp 
////////////////////////////////////////////////////////////////////////////////////
...
class LZ_Detector : public VDetector {
 public:
  ...
  virtual void Initialization() override {
    ...
    g1 = 0.113569;// 0.114 +/- 0.002  // phd per S1 phot at dtCntr (not phe). Divide out 2-PE effect
    ...
    numPMTs = 484; //Taking into account turned-off PMTs    // For coincidence calculation
    ...
    g1_gas = 0.092103545;// 0.092 +/- 0.002 // phd per S2 photon in gas, used to get SE size
    s2Fano = 2.0;  // Fano-like fudge factor for SE width
    s2_thr = 600. * (1.0 + P_dphe);  // the S2 threshold in phe or PE, *not* phd. Affects NR most
    ...
    dtCntr = 462.5; // central correction bin is between 425-500us // center of detector for S1 corrections, in usec.
    dt_min = 67.; //minimum. Top of detector fiducial volume
    dt_max = 945.;  // maximum. Bottom of detector fiducial volume
  ...
  //The following functions were not used in LZ's SR1, and are copied from the public NEST
